...
def task1_sync_calculations():
    ...
    # Операция 1: 15 + 25 с задержкой 2 секунды
    result1 = sync_calculate('+', 15, 25, 2)
    results.append(result1)

    # Операция 2: 40 - 18 с задержкой 1 секунда
    result2 = sync_calculate('-', 40, 18, 1)
    results.append(result2)

    # Операция 3: 12 * 8 с задержкой 3 секунды
    result3 = sync_calculate('*', 12, 8, 3)
    results.append(result3)

    # Операция 4: 100 / 5 с задержкой 1 секунда
